...
#Модель
Обозначим предсказываемую, зависимую переменную как $$y$$. $$y$$ может принимать два значения $$0$$ событие не произошло и $$1$$ событие произошло. 
...
![Logistic regression image](/assets/logistic_regression/Logistic-curve.png){: style="text-align: center"}
...
# В нашем случае все независимые переменные очень важны
#Coefficients:
...
Результатом логистической регрессии является вероятность, но чтобы сделать предсказание, нам необходимо определить пороговое (`threshold`) значение, такое что:

$$P(PoorCare = 1) >= t$$, событие произошло

$$P(PoorCare = 1) < t$$, событие не произошло 

При высоком пороговом значении, модель будет редко предсказывать положительный исход (при высокой вероятности $$P(y=1)$$).
...
В таблице мы видим как часто наша модель делает правильные предсказания и ошибается:
...
$$Specificity = TN / (TN + FP)$$ - специфичности, или `True negative rate`. Процент верно предсказанных негативных исходов.
...
#  Модель верно предсказывает отрицательный исход 454 случаях из 594
