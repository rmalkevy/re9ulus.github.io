...
# Получаем подстроку
> substr("Select this word!", start = 8, stop = 16)
[1] "this word"
...
# Печатаем строку
> sprintf("Who is has %s?", "John Galt")
...
#Определяем тип переменной и конвертируем типы

Что бы проверить, принадлежит ли переменная к какому-либо типу можно использовать функции вида `is.*`:
...
# использовать другой пример, например со странами, или книгами
{% highlight R %}
# Вектор строк
